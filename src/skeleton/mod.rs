...
impl VertexType {
    ...
    fn new_root_vertex(location: Coordinate, time_elapsed: f32) -> Self {
    ...
    #[allow(dead_code)]
    fn initialize_from_polygon(input_polygon: &Polygon<f32>, orient: bool) -> Vec<Self> {
        Self::initialize_from_polygon_vector(&vec![input_polygon.clone()], orient)
    }

    fn initialize_from_polygon_vector(
        input_polygon_vector: &Vec<Polygon<f32>>,
        orient: bool,
    ) -> Vec<Self> {
        let mut ret = Vec::new();
...
impl Skeleton {
    pub(crate) fn apply_vertex_queue(
        ...
        for ls in &lsv {
            if ls.winding_order() == Some(WindingOrder::CounterClockwise) {
                let p1 = Polygon::<f32>::new(ls.clone(), vec![]);
                res.push(p1);
            }
        }
        for ls in &lsv {
    ...
    pub(crate) fn get_vertex_queue(&self, time_elapsed: f32) -> VertexQueue {
    ...
    fn find_split_vertex(
        ...
        is_init: bool,
        orient: bool,
    ) -> Vec<(f32, Coordinate, IndexType, usize)> {
        let mut ret = Vec::new();
    ...
    pub(crate) fn skeleton_of_polygon(input_polygon: &Polygon<f32>, orient: bool) -> Self {
    ...
    pub(crate) fn to_linestring(&self) -> Vec<LineString<f32>> {
        fn dfs_helper(
            cur: usize,
            visit: &mut Vec<bool>,
            ret: &mut Vec<LineString<f32>>,
            ray_vector: &Vec<VertexType>,
        ) {
            if visit[cur] {
                return;

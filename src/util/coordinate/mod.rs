...
impl From<Coordinate> for (f32, f32) {
    fn from(item: Coordinate) -> (f32, f32) {
        (item.0, item.1)
    }
}

impl From<(f32, f32)> for Coordinate {
    fn from(item: (f32, f32)) -> Coordinate {
        Coordinate(item.0, item.1)
    }
}

impl From<geo_types::Coord<f32>> for Coordinate {
    fn from(value: geo_types::Coord<f32>) -> Self {
        Coordinate(value.x, value.y)
    }
}

impl From<Coordinate> for geo_types::Coord<f32> {
    fn from(value: Coordinate) -> geo_types::Coord<f32> {
...
impl Coordinate {
    ...
    ///
    pub fn outer_product(&self, rhs: &Self) -> f32 {
    ...
    /// ```
    pub fn dist_coord(&self, rhs: &Coordinate) -> f32 {
        f32::sqrt((self.0 - rhs.0) * (self.0 - rhs.0) + (self.1 - rhs.1) * (self.1 - rhs.1))
    }
    ...
    /// ```
    ///
    pub fn dist_ray(&self, rhs: &Ray) -> f32 {
        if rhs.is_degenerated() {
            return self.dist_coord(&rhs.origin);
        }
        return f32::abs((*self - rhs.origin).outer_product(&rhs.angle)) / rhs.angle.norm();
    }

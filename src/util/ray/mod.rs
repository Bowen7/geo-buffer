...
impl Ray {
    ...
    /// ```
    pub fn point_by_ratio(&self, ratio: f32) -> Coordinate {
    ...
    /// let c2 = (3., 4.).into();
    /// let r1 = Ray::new(c1, c2);
    /// let r2 = r1.rotate_by(std::f32::consts::PI/2.);
    ///
    /// assert!(r2.point_by_ratio(1.).eq(&(-4., 3.).into()));
    /// ```
    pub fn rotate_by(&self, angle: f32) -> Self {
        let nx = self.angle.0 * f32::cos(angle) - self.angle.1 * f32::sin(angle);
        let ny = self.angle.0 * f32::sin(angle) + self.angle.1 * f32::cos(angle);
        Self {
            origin: self.origin,
            angle: (nx, ny).into(),
        }
